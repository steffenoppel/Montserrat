...
`r format(min(surveys$Date),format="%d %b")` and
`r format(max(surveys$Date),format="%d %b")` and completed a total of
...
`r min(totals$N)` and `r max(totals$N)` birds of
...
ggplot()+
geom_line(aes(x=Year, y=mean,col=col), size=1)+facet_wrap(~fullspec, ncol=2, scales="free_y")+
...
table2<-trendout %>% filter(parameter == 'trend') %>% # I assume trend is the actual trend and not the estimate of detection prob
